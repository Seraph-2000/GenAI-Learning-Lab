...
def planner(client, model, input):
    ...
    router_prompt = f"""
        ...
        {{
            ...
            "tool":"calculator",
            "query":"25 * 89",
            "reason":"Need arithmetic calculation"
        }}
        ]
        ...
        {{
            ...
            "tool":"search_docs",
            "query":"refund policy",
            "reason":"Need company policy information"
        }}
        ]
...
def main():
    ...
    while True:
        ...
        planner_response = planner(client=client, model=gen_model, input=query)
        plan = json.loads(planner_response.text)
        plan.sort(key=lambda step: step["step"])

        all_results = []

        for step in plan:
            results = execute_tool(tool_call=step, model=emb_model, documents=documents, document_embeddings=document_embeddings)
            ...
            if step["tool"] == "calculator":
                ...
                if results == "Invalid expression":
                    ...
                    repaired_query = repair_expression(client=client, model=gen_model, query=step["query"])
