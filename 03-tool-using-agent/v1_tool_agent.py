...
def llm_route(client, model, input):
    ...
    router_prompt = """
        ...
        {"tool":"calculator","query":"25 * 89"}

        {"tool":"search_docs","query":"What is the refund policy?"}
...
def main():
    ...
    while True:
        ...
        tool_call = json.loads(route_response.text)
        ...
        if tool_call["tool"] == "calculator":
            history.append({"role" : "Assistant", "content" : str(results)})
            print(results)
